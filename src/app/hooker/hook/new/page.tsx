// src/app/hooker/hook/new/page.tsx
'use client'
...
// Link types - ALL unlocked for logged-in users
const LINK_TYPES = [
  { id: 'know_more', label: 'Know More', icon: Link2, placeholder: 'https://anylink.com/your-page', color: 'from-purple-500 to-pink-500' },
  { id: 'instagram', label: 'Instagram', icon: InstagramIcon, placeholder: 'https://instagram.com/yourhandle', color: 'from-purple-500 to-pink-500' },
  { id: 'youtube', label: 'YouTube', icon: YoutubeIcon, placeholder: 'https://youtube.com/@yourchannel', color: 'from-red-500 to-red-600' },
  { id: 'website', label: 'Website', icon: Globe, placeholder: 'https://yourwebsite.com', color: 'from-blue-500 to-indigo-500' },
  { id: 'store', label: 'Store', icon: ShoppingBag, placeholder: 'https://yourstore.com', color: 'from-emerald-500 to-teal-500' },
]
...
export default function CreateHookPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!user) {
        router.push('/hooker/login?redirect=' + encodeURIComponent('/hooker/hook/new'))
        return
      }
      setUser(user)
  ...
  // ALL links are free for logged-in users
  ...
  const progress = [
    ...
    title.length > 0,
    category.length > 0,
    true, // logged in, no email needed
  ...
  // Success screen with floating images
      ...
      <div className="min-h-screen bg-white relative overflow-hidden flex items-center justify-center px-4 py-20">
        ...
        <div className={`relative z-10 max-w-md w-full text-center transition-all duration-1000 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
          ...
          <p className="text-sm text-neutral-400 mb-10">
            Posted by {user?.user_metadata?.full_name || 'You'}
          </p>
            ...
            <div className="bg-white/90 backdrop-blur-xl rounded-3xl border border-white/60 shadow-2xl shadow-purple-500/10 overflow-hidden">
              ...
              {step === 2 && (
                <div className="p-8 pt-2 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
                  ...
                  {/* Links - ALL UNLOCKED for logged in */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-neutral-700 flex items-center gap-2">
                    ...
                    <p className="text-xs text-purple-600 font-medium mb-2 flex items-center gap-1">
                      <Sparkles className="w-3 h-3" /> All links unlocked for you
                    </p>
                      ...
                      {LINK_TYPES.map((linkType) => {
                        ...
                        return (
                          <button 
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border-2 transition-all ${isAdded ? 'bg-gradient-to-r ' + linkType.color + ' text-white border-transparent shadow-md' : 'bg-white text-neutral-600 border-neutral-200 hover:border-purple-300 hover:bg-purple-50'}`}
                          >
                    ...
                    <div className="bg-white rounded-xl p-4 border border-purple-100">
                      <div className="flex items-center gap-3">
                        ...
                        <Badge className="ml-auto bg-green-100 text-green-700 border-0">
                          <User className="w-3 h-3 mr-1" /> Verified
                        </Badge>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between pt-4">
                    ...
                    <Button onClick={() => setStep(3)} disabled={!title || !category} className="bg-gradient-to-r from-purple-600 to-pink-500 text-white rounded-full h-12 px-8 gap-2 disabled:opacity-40 hover:shadow-lg hover:shadow-purple-500/20 transition-all">
