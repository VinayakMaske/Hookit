...
// Custom Twitter/X icon since lucide-react removed it
...
export default function HookDetailPage() {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setCurrentUserName(data.user?.name || data.user?.email?.split('@')[0] || '')
      }
    } catch {
      setIsLoggedIn(false)
    }
  ...
  const redirectToLogin = () => {
    ...
    router.push('/hooker/login?redirect=' + encodeURIComponent(`/hook/${hookId}`))
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      {showMobileComments && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm sm:hidden" onClick={() => setShowMobileComments(false)}>
          <div className="bg-white rounded-t-3xl w-full max-h-[calc(85vh-80px)] mb-20 flex flex-col shadow-2xl animate-in slide-in-from-bottom-10 duration-300"onClick={e => e.stopPropagation()}>
        ...
        <div className="grid lg:grid-cols-[1fr_420px] gap-6 lg:gap-10">
          ...
          <div className="space-y-4">
            <div className="relative rounded-3xl overflow-hidden bg-neutral-100 group cursor-pointer" onClick={handleImageClick}>
              ...
              {images.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); setCurrentImageIndex(prev => prev === 0 ? images.length - 1 : prev - 1); setImageBlurred(false) }} className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:scale-110 z-10">
                    <ChevronLeft className="w-5 h-5 text-neutral-700" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); setCurrentImageIndex(prev => prev === images.length - 1 ? 0 : prev + 1); setImageBlurred(false) }} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 backdrop-blur-sm shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:scale-110 z-10">
          ...
          {/* RIGHT: Details Panel - Pinterest Style (MATCHES LOGGED-IN VERSION) */}
          <div className="space-y-4 lg:pt-2">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-sm text-neutral-500">{hook.creator_email || 'Creator'}</p>
                </div>
              </div>
            ...
            {hook.external_links && hook.external_links.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {hook.external_links.map((link, i) => {
                  ...
                  return (
                    <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white bg-gradient-to-r ${color} shadow-sm hover:shadow-md transition-shadow`}>
            ...
            <div className="hidden sm:block border-t border-neutral-100 pt-4">
              ...
              <div ref={commentsScrollRef} className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                {comments.length === 0 && (
      ...
      {related.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-4 space-y-4">
            {related.map((item) => (
              <Link key={item.id} href={`/hook/${item.id}`} className="block break-inside-avoid">
                <div
