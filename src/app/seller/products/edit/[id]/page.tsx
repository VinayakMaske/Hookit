...
export default function EditProductPage() {
    ...
    const fetchProduct = async () => {
        ...
        setFormData({
            ...
            additionalFee: data.additional_fee?.toString() || '',
            additionalFeeName: data.additional_fee_name || '',
        })

        setHasAffiliate(!!data.affiliate_link)
        setHasDeliveryFee(!!data.delivery_fee && data.delivery_fee > 0)
        setHasPlatformFee(!!data.platform_fee && data.platform_fee > 0)
        setHasAdditionalFee(!!data.additional_fee && data.additional_fee > 0)
        setGstType(data.gst_type || 'none')
        setGstSlab(data.gst_slab || 'none')
        setCustomGstPercentage(data.gst_percentage && !GST_SLABS.find(s => s.percentage === data.gst_percentage) ? data.gst_percentage.toString() : '')

        if (data.images && data.images.length > 0) {
            const existingImages: ImageFile[] = data.images.map((url: string, index: number) => ({
                id: `existing-${index}`,
                file: null as any,
                preview: url,
                url: url,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error: updateError } = await supabase
            ...
            .update({
                ...
                additional_fee_name: hasAdditionalFee ? formData.additionalFeeName : null,
                // GST
                gst_type: gstType,
