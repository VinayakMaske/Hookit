...
// Custom SVG Icons (lucide-react doesn't have Instagram/YouTube)
...
export default function HookDetailPage() {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      // Replace with your actual auth check (Supabase, NextAuth, etc.)
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    if (!commentText.trim() || !commentName.trim() || !hook) return
    ...
    try {
      const res = await fetch(`/api/hooks/${hookId}/comments`, {
        ...
        body: JSON.stringify({
          ...
          author_email: 'anonymous@hookit.online',
        }),
      })
      if (res.ok) {
                ...
                <form onSubmit={handleSubmitComment} className="space-y-3">
                  <div className="flex gap-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center text-white font-bold shrink-0">
                      {commentName ? commentName[0].toUpperCase() : '?'}
                    </div>
                    <div className="flex-1 space-y-2">
                      <Input
                        ...
                        className="rounded-xl border-neutral-200 focus:border-purple-300 h-10 text-sm"
                        required
                      />
